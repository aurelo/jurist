...
-------------------------------------------------------------------------------
    ...
    begin
      ...
      loop
        v_seq_id := novi_dug(p_iznos_duga => r_dugovi.iznos, p_datum_duga => r_dugovi.datum_dospijeca, p_tip_duga_id => r_dugovi.vrsta_transakcije_id, p_na_osnovu_transakcije_id => r_dugovi.transakcija_id);
      end loop;
    end;
-------------------------------------------------------------------------------
    ...
    begin
      ...
      loop
          v_seq_id := nova_uplata(p_iznos_uplate => r_uplate.iznos, p_datum_uplate => r_uplate.datum_uplate, p_tip_uplate_id => r_uplate.vrsta_transakcije_id, p_na_osnovu_transakcije_id => r_uplate.transakcija_id);
      end loop;
    end;

-- helper functions --
    function get_session_dug_id(
      p_na_osnovnu_transakcije_id      in    number
    )
    return apex_collections.seq_id%type
    is
    cursor cur_dug_seq_id
    is
      select   seq_id
      from     ju_session_dugovi_v d
      where    d.na_osnovu_transakcije_id = p_na_osnovnu_transakcije_id
      ;
      v_seq_id    apex_collections.seq_id%type;
    begin
      open cur_dug_seq_id;

      fetch cur_dug_seq_id
      into  v_seq_id;

      close cur_dug_seq_id;

      return v_seq_id;
    end;

    function get_session_uplata_id(
      p_na_osnovnu_transakcije_id      in    number
    )
    return apex_collections.seq_id%type
    is
      cursor cur_uplata_seq_id
      is
      select   seq_id
      from     ju_session_uplate_v u
      where    u.na_osnovu_transakcije_id = p_na_osnovnu_transakcije_id
      ;

      v_seq_id    apex_collections.seq_id%type;
    begin
      open cur_uplata_seq_id;

      fetch cur_uplata_seq_id
      into  v_seq_id;

      close cur_uplata_seq_id;
...
--                         ,p_n002                => r_rezultati.uplata_id
                         ,p_n001                => get_session_dug_id(r_rezultati.dug_id)
                         ,p_n002                => get_session_uplata_id(r_rezultati.uplata_id)
...
-------------------------------------------------------------------------------
    procedure  save_dugovi_za_izracun(
       p_izracun_id          in      ju_izracun_zatezne.id%type
    )
    is
    begin
...
--       ,        sri.uplata_id
       ,       (
                ...
                where    jte.ize_id = p_izracun_id
                and      jte.session_transakcija_id = sri.dug_id
                and      vta.id = jte.vta_id
                and      vta.strana = 'D'
               )
       ,       (
                select   jte.id
                ...
                ,        ju_vrste_transakcija vta
                where    jte.ize_id = p_izracun_id
                and      jte.session_transakcija_id = sri.uplata_id
                and      vta.id = jte.vta_id
                and      vta.strana = 'P'
               )
...
-------------------------------------------------------------------------------
  ...
  begin
    ...
    return apex_collection.add_member(p_collection_name => DUGOVI,
                                      ...
                                      p_n003            => p_na_osnovu_transakcije_id
                                     );
  end;
-------------------------------------------------------------------------------
...
-------------------------------------------------------------------------------
   ...
   ,p_opis            in    varchar2
    ...
    begin
      ...
      save_dugovi_za_izracun(v_izracun_id);
...
-------------------------------------------------------------------------------
  ...
  is
    ...
    v_dug_min_datum     date;
    v_dug_max_datum     date;
    
    v_uplata_min_datum  date;
    v_uplata_max_datum  date;

    cursor cur_ks_ekstremi
    is
    select   min(jks.ks_datum_od)
    ,        max(jks.ks_datum_do)
    from     ju_kamatne_stope_izracuna_v jks
    where    jks.id_tipa_izracuna = p_tip_izracuna
    ;
    
    cursor cur_dug_ekstremi
    is
      select   min(sd.datum_dospijeca)
      ,        max(sd.datum_dospijeca)
      from     ju_session_dugovi_v sd
    ;
    
    cursor cur_uplate_ekstremi
    is
      select   min(su.datum_uplate)
      ,        max(su.datum_uplate)
      from     ju_session_uplate_v su
    ;
  ...
  begin
     ---ks
     open cur_ks_ekstremi;
     
     fetch cur_ks_ekstremi
     into  v_ks_min_datum
     ,     v_ks_max_datum
     ;
     
     close cur_ks_ekstremi;
     
     
     --- dug
     open cur_dug_ekstremi;
     
     fetch cur_dug_ekstremi
     into  v_dug_min_datum
     ,     v_dug_max_datum
     ;
     
     close cur_dug_ekstremi;
     
     -- uplate
     open cur_uplate_ekstremi;
     
     fetch cur_uplate_ekstremi
     into  v_uplata_min_datum
     ,     v_uplata_max_datum;
     
     close cur_uplate_ekstremi;
     
     
     if p_datum_izracuna > v_ks_max_datum
     then
        v_err := 'Datum izračuna: '||to_char(p_datum_izracuna, 'dd.mm.yyyy')||' je veći od zadnje definirane kamatne stope: '||to_char(v_ks_max_datum, 'dd.mm.yyyy')||'!';
     end if;
     
     if p_datum_izracuna < v_ks_min_datum
     then
        v_err := v_err||'Datum izračuna: '||to_char(p_datum_izracuna, 'dd.mm.yyyy')||' je manji od prve definirane kamatne stope: '||to_char(v_ks_min_datum, 'dd.mm.yyyy')||'!';
     end if;
     
     if v_dug_max_datum > v_ks_max_datum
     then
        v_err := 'Datum duga: '||to_char(v_dug_max_datum, 'dd.mm.yyyy')||' veći je od zadnje definirane kamatne stope: '||to_char(v_ks_max_datum, 'dd.mm.yyyy')||'!';
     end if;
     
     if v_dug_min_datum < v_ks_min_datum
     then
        v_err := 'Datum duga: '||to_char(v_dug_min_datum, 'dd.mm.yyyy')||' manji je od prve definirane kamatne stope: '||to_char(v_ks_min_datum, 'dd.mm.yyyy')||'!';
     end if;
